...
@Slf4j
@Component
public class MerchantJwtTokenInterceptor implements HandlerInterceptor {
    private final JwtProperties jwtProperties;
    @Autowired
    public MerchantJwtTokenInterceptor(JwtProperties jwtProperties) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
        ...
        try{
            String secretKey = jwtProperties.getMerchantSecretKey();
            Jws<Claims> jws = JwtUtil.parseJwt(secretKey, token);
            Claims claims = jws.getPayload();
            request.setAttribute("claims:", claims);
            Long id = claims.get(JwtClaimsConstant.MERCHANT_ID, Long.class);
            log.info("当前管理员： {}", id);
            return true;
        }
        catch (Exception e) {
